...
class database //Object database spanning multiple files
{
	...
	static void switchobj(const char* name); //Jump to the named object in the database, returning if it was found or not
	static char* getvalue(const char* key,char* val); //Get the value of the given attribute (space to put value should be specified)
	...
	static void putvalue(const char* key,long val);

	private:
	static void submitobj(FILE* strm,const char* nam,long pos,long len); //Submit an object to the location hash, detailing which file it's in, its name, its position within the file and length within the file (strm is a filestream, dodgy workaround necessitates 'void')
